...
void Command_Parse(void) {
    ...
    /* 读取LDC寄存器 */
        ...
        if (ldc2_isReading == false) {
            ...
            uint8_t reg_val = ldc1101_readByte(&hldc1, reg_addr);
    ...
    /* LDC状态检查 */
        ...
        if (ldc2_isReading == false) {
            uint8_t status = isLHR ? ldc1101_readByte(&hldc1, _LDC1101_REG_LHR_STATUS)
                                   : ldc1101_readByte(&hldc1, _LDC1101_REG_RP_L_MEASUREMENT_STATUS);

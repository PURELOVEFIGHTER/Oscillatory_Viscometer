...
/* LDC1101 相关变量 */
LDC1101_HandleTypeDef ldc1  = {&hspi1, LDC1_CS_GPIO_Port, LDC1_CS_Pin};
...
void Command_Parse(void) {
    ...
    // 检查 RP+L 模式工作状态
        ...
        ldcStatus[0] = ldc1101_readByte(&ldc2, _LDC1101_REG_RP_L_MEASUREMENT_STATUS);

        // 将 rpl_status 转成二进制字符串
        sprintf(TX_BUFFER,
                "RP+L Measurement Status: "
                "%d%d%d%d%d%d%d%d\r\n",
                (ldcStatus[0] >> 7) & 0x01, (ldcStatus[0] >> 6) & 0x01, (ldcStatus[0] >> 5) & 0x01,
                (ldcStatus[0] >> 4) & 0x01, (ldcStatus[0] >> 3) & 0x01, (ldcStatus[0] >> 2) & 0x01,
                (ldcStatus[0] >> 1) & 0x01, (ldcStatus[0] >> 0) & 0x01);
        ...
        sprintf(TX_BUFFER,
                ...
                "%d%d%d%d%d%d%d%d\r\n",
                (ldcStatus[1] >> 7) & 0x01, (ldcStatus[1] >> 6) & 0x01, (ldcStatus[1] >> 5) & 0x01,
                (ldcStatus[1] >> 4) & 0x01, (ldcStatus[1] >> 3) & 0x01, (ldcStatus[1] >> 2) & 0x01,
                (ldcStatus[1] >> 1) & 0x01, (ldcStatus[1] >> 0) & 0x01);
...
int main(void) {
    ...
    // LDC1101 初始化
    if (ldc1101_init(&ldc1, _LDC1101_RP_SET_RP_MIN_1_5KOhm) || ldc1101_init(&ldc2, _LDC1101_RP_SET_RP_MIN_1_5KOhm)) {

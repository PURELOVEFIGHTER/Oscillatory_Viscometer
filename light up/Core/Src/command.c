...
void Command_Parse(void) {
    ...
    if (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "DR:", 3) == 0) {
        ...
        /* SPI检查 */
    } else if (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "SPI Check", 9) == 0) {
        HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
        uint8_t chip_id = ldc1101_readByte(&ldc2, _LDC1101_REG_CHIP_ID);
        sprintf(UART1_TX_buffer, "LDC1101 Device ID: 0x%02X\r\n", chip_id);

        /* 读取LDC寄存器 */
    } else if (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "Read:", 5) == 0) {
        uint8_t reg_addr;
        ...
        /* LDC状态检查 */
               ...
               || (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "LHR Check", 9) == 0 && isLHR)) {
        HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);

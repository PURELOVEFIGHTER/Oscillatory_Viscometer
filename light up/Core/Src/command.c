#include "command.h"
#include "ldc1101_driver.h"
#include "tim.h"
#include <string.h>
#include <stdlib.h>
#include <stdio.h>

extern UART_HandleTypeDef huart1;
extern char UART1_RX_DMA_buffer[2][MSG_LEN];
extern char UART1_TX_buffer[MSG_LEN];

extern uint16_t drv_PWM_freq;
extern uint16_t drv_PWM_DR;

extern LDC1101_HandleTypeDef ldc2;
extern bool isLHR;
extern uint8_t LDC_status;

void Command_Parse(void) {
    if (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "DR:", 3) == 0) // 设置占空比
    {
        drv_PWM_DR = atoi(&UART1_RX_DMA_buffer[UART1_RX_activeBuffer][3]);
        HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
        sprintf(UART1_TX_buffer, "PWM_DR set to %d\r\n", drv_PWM_DR);
    } else if (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "FR:", 3) == 0) // 设置频率
    {
        drv_PWM_freq = atoi(&UART1_RX_DMA_buffer[UART1_RX_activeBuffer][3]);
        HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
        sprintf(UART1_TX_buffer, "PWM_FREQ set to %dHz\r\n", drv_PWM_freq);
    } else if (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "FR Sweep", 8) == 0) {
        freq_sweep_enabled = true;
        drv_PWM_freq       = FREQ_SWEEP_START_HZ;
        HAL_TIM_Base_Start_IT(&htim4);
        sprintf(UART1_TX_buffer, "Frequency sweeping start.\r\n");
    } else if (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "SPI Check", 9) == 0) // SPI检查
    {
        HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
        uint8_t chip_id = ldc1101_readByte(&ldc2, _LDC1101_REG_CHIP_ID);
        sprintf(UART1_TX_buffer, "LDC1101 Device ID: 0x%02X\r\n", chip_id);
    } else if (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "Read:", 5) == 0) // 读取寄存器
    {
        uint8_t reg_addr;
        sscanf(&UART1_RX_DMA_buffer[UART1_RX_activeBuffer][5], "%hhx", &reg_addr);
        uint8_t reg_val = ldc1101_readByte(&ldc2, reg_addr);
        sprintf(UART1_TX_buffer, "[0x%02X] = 0x%02X\r\n", reg_addr, reg_val);
    } else if ((strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "RPL Check", 9) == 0 && !isLHR)
               || (strncmp(UART1_RX_DMA_buffer[UART1_RX_activeBuffer], "LHR Check", 9) == 0 && isLHR)) // 状态检查
    {
        HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
        uint8_t status = isLHR ? ldc1101_readByte(&ldc2, _LDC1101_REG_LHR_STATUS)
                               : ldc1101_readByte(&ldc2, _LDC1101_REG_RP_L_MEASUREMENT_STATUS);
        sprintf(UART1_TX_buffer, "%s Measurement Status: %d%d%d%d%d%d%d%d\r\n", isLHR ? "LHR" : "RP+L",
                (status >> 7) & 1, (status >> 6) & 1, (status >> 5) & 1, (status >> 4) & 1, (status >> 3) & 1,
                (status >> 2) & 1, (status >> 1) & 1, (status >> 0) & 1);
    } else {
        sprintf(UART1_TX_buffer, "Unknown command\r\n");
    }
    HAL_UART_Transmit(&huart1, (uint8_t *)UART1_TX_buffer, strlen(UART1_TX_buffer), HAL_MAX_DELAY);
}
